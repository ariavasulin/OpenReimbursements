...
test('AC-16/AC-17: a three-level tree imports with no edits to names, and a top-level choice reaches the folders inside', async ({ page, context }, info) => {
  ...
  await inside.getByRole('combobox', { name: 'Tags for every folder in Smith Residence', exact: true }).click();
  await inside.getByRole('option', { name: 'professional', exact: true }).click();
...
test('AC-17: 5,000 folder rows stay usable: review scrolls, finds, and edits', async ({ page, context }, info) => {
  ...
  await everything.getByRole('combobox', { name: `Tags for every folder in ${root}`, exact: true }).click();
  const tagged = Date.now();
  await everything.getByRole('option', { name: 'field dimension', exact: true }).click();

...
import { describeAttemptResult, uploadRpc } from './uploads';
...
export async function ingestMigrationChunk(actor: PhotoActor, source: string, body: Body) {
  ...
  return uploadRpc(actor, 'migration_chunk', { p_actor: actor.actorId, p_source: photoId(source), p_scan: photoId(body.scan_id),
...
export async function saveMigrationSource(actor: PhotoActor, batch: string, body: Body) {
  ...
  return uploadRpc(actor, 'migration_source', { p_actor: actor.actorId, p_batch: photoId(batch), p_source: body.id ? photoId(body.id) : randomUUID(),
...
export async function readMigrationBatch(actor: PhotoActor, id: string) {
  const summary = await readPhotoBatch(actor, id, 'migration');
  const { data: details, error } = await actor.db.from('migration_batches').select('script_name,origin').eq('id', id).single();
  if (error) throwPhotoDatabaseError(error);
  let requested_input = {};
  if (summary.can_mutate && details.origin === 'mcp') {
  ...
  return { ...summary, batch: { ...(summary.batch as unknown as Body), ...details, requested_input },
    counts: await uploadRpc(actor, 'migration_counts', { p_actor: actor.actorId, p_batch: id }) };
}
...
export async function readMigrationItems(actor: PhotoActor, id: string, request: Request) {
  await readPhotoBatch(actor, id, 'migration');
...
export async function migrationItemAction(actor: PhotoActor, id: string, body: Body) {
  if (body.action === 'outcome') {
    ...
    return uploadRpc(actor, 'migration_item_outcome', { p_actor: actor.actorId, p_item: photoId(id), p_retry: retry,
  ...
  return uploadRpc(actor, 'migration_item_action', { p_actor: actor.actorId, p_item: photoId(id), p_action: body.action,

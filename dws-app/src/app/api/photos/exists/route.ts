...
import { PhotoApiError, photoJson, photoRoute, photoRpc } from '@/lib/photos/server/http';
import { describeUploadOutcome } from '@/lib/photos/server/uploads';
...
export async function GET(request: Request) {
  return photoRoute(async () => {
    ...
    const outcome = await photoRpc(actor, 'photo_canonical_outcome', { p_digest: sha });

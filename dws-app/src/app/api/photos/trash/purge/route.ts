...
import { BULK_BODY_BYTES, photoId } from '@/lib/photos/server/reads';
import { MAX_BULK_PHOTOS } from '@/lib/photos/apiShared';
import type { PurgeMarked } from '@/lib/photos/action-types';
...
const ids = (value: unknown): string[] => {
  ...
  if (!Array.isArray(value) || value.length > MAX_BULK_PHOTOS) throw new PhotoApiError('invalid_input');
...
export async function POST(request: Request) {
  return photoRoute(async () => {
    ...
    let marked: PurgeMarked = { photos: 0, albums: 0, projects: 0 };

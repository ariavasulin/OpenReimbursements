...
import { PhotoApiError, photoJson, photoRoute, readPhotoJson } from '@/lib/photos/server/http';
import { photoId } from '@/lib/photos/server/reads';
import { createUploadAttempt, describeAttemptResult, uploadRpc } from '@/lib/photos/server/uploads';
...
export async function POST(request: Request) {
  return photoRoute(async () => {
    ...
    try {
      result = await uploadRpc(actor, 'photo_cancel_upload', args);
    } catch (error) {
    ...
    const bound = await uploadRpc(actor, 'photo_lock_upload', {

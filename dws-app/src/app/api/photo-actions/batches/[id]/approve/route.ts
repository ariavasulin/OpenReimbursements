...
import { actionRpc, onlyKeys, readActionBatch } from '@/lib/photos/server/actions';
export async function POST(request:Request,context:{params:Promise<{id:string}>}){return photoRoute(async()=>{
 ...
 await actionRpc(actor,'photo_approve_action',{p_actor:actor.actorId,p_batch_id:id});

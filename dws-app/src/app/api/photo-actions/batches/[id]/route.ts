...
import { actionIds, actionPage, onlyKeys, readActionBatch } from '@/lib/photos/server/actions';
...
export async function PATCH(request:Request,context:Context){return photoRoute(async()=>{
 ...
 await photoRpc(actor,'photo_action_control',{p_actor:actor.actorId,p_batch_id:id,p_action:body.action,p_photo_ids:body.action==='skip'?actionIds(body.photo_ids):[]});
